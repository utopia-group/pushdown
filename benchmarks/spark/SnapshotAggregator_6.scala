...
object SnapshotAggregator_6 {
  def main(args: Array[String]): Unit = {
    val spark = SparkSession.builder
      .appName("SnapshotAggregator_6")
      .getOrCreate()
    ...
    val filtered = exploded.filter(
      (($"offset" > 10 && $"offset" < 14) || $"offset" > 16 || $"offset" === 15) &&
      !$"value".isNull && $"value" >= 1 && $"value" =!= 29)
    ...
    val filtered_pre = exploded_pre.filter(
      $"offset" =!= -1 && ($"offset" < 14 || $"offset" > 16 || $"offset" === 15) &&
      ($"value".isNull || ($"value" >= 1 && $"value" =!= 29)))
