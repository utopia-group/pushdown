...
object RoaringBitmapUDAF_2 {
  def main(args: Array[String]): Unit = {
    val spark = SparkSession.builder
      .appName("RoaringBitmapUDAF_2")
      .getOrCreate()
    ...
    def covered(bm: RoaringBitmap, k: Int): Boolean = {
      val left  = 2 * k               // “start” sentinel for value k
      val right = 2 * k - 1           // “end” sentinel, negative for k = 0
      bm.contains(left) || (right >= 0 && bm.contains(right))
    }
    def bitVal(bm: RoaringBitmap, k: Int): Int =
      if (covered(bm, k)) 1 else 0        // our bitmap is 0/1-valued per position
