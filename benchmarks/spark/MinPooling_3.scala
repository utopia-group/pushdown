...
object MinPooling_3 {
  def main(args: Array[String]): Unit = {
    ...
    val filtered = out.filter(
      !$"row".getItem(0).isNull && ($"row".getItem(0) <= 32 || $"row".getItem(0) === 50) &&
      ((!$"row".getItem(1).isNull && $"row".getItem(1) > 100) ||
       (!$"row".getItem(2).isNull && $"row".getItem(2) =!= 15 && $"row".getItem(2) <= 29)))
