...
object AggregationQuerySuite_6 {
  def main(args: Array[String]): Unit = {
    val spark = SparkSession.builder
      .appName("AggregationQuerySuite_6")
      .getOrCreate()
    ...
    val filtered = exploded.filter(
      (!$"key".isNull && $"key" >= 40) ||
      (!$"value1".isNull && $"value1" >= 22 && $"value1" =!= 44 && $"value1" < 50) ||
      (!$"value2".isNull && $"value2" > 44 && $"value2" =!= 50 && $"value2" <= 60))
    ...
    val filtered_pre = exploded_pre.filter(
      $"key".isNotNull ||
      ($"value1" >= 22 && $"value1" =!= 44 && $"value1" < 50) ||
      ($"value2" > 44 && $"value2" =!= 50 && $"value2" <= 60))
