...
object ReturnPriceAggregator_4 {
  def main(args: Array[String]): Unit = {
    val spark = SparkSession.builder
      .appName("ReturnPriceAggregator_4")
      .getOrCreate()
    ...
    val filtered = exploded.filter(
      $"openPrice" <= 100 &&
      !$"openEpoch".isNull &&
      ($"openEpoch" < 38 || $"openEpoch" === 40) &&
      $"closePrice" >= 10 &&
      !$"closeEpoch".isNull &&
      ($"closeEpoch" === 53 || $"closeEpoch" >= 55))
    ...
    val filtered_pre = exploded_pre.filter(
      ...
      $"closePrice" >= 10 &&
      ($"openEpoch" < 38 || $"openEpoch" === 40) &&
      ($"closeEpoch".isNull || $"closeEpoch" === 53 || $"closeEpoch" >= 55))
