...
object KeepRowWithMaxQuality_5 {
  def main(args: Array[String]): Unit = {
    val spark = SparkSession.builder
      .appName("KeepRowWithMaxQuality_5")
      .getOrCreate()
    ...
    val filtered = exploded.filter(
      ...
      ((!col(udaf.KEY_QUALITY_SCORE).isNull && col(udaf.KEY_QUALITY_SCORE) > 90) ||
    ...
    val filtered_pre = exploded_pre.filter(
      ((col(udaf.KEY_QUALITY_SCORE).isNull || col(udaf.KEY_QUALITY_SCORE) > 90) ||
       (col(udaf.KEY_S_SEQUENCE).isNull || col(udaf.KEY_S_SEQUENCE) =!= 200)) &&
      (col(udaf.KEY_SEQUENCE_IDENTIFIER).isNull || col(udaf.KEY_SEQUENCE_IDENTIFIER) > 1) &&
      (col(udaf.KEY_QUALITY_SCORE).isNotNull || col(udaf.KEY_S_SEQUENCE).isNotNull))
