...
object EventAggregationUdf_9 {
  def main(args: Array[String]): Unit = {
    val spark = SparkSession.builder
      .appName("EventAggregationUdf_9")
      .getOrCreate()
    ...
    val mid1 = to_timestamp(lit("1990-07-30 08:00:00"), "yyyy-MM-dd HH:mm:ss")
    val mid2 = to_timestamp(lit("1995-07-30 08:00:00"), "yyyy-MM-dd HH:mm:ss")
    ...
    val filtered = exploded.filter(
      $"time" > 0 && ($"price" > 5 && $"price" <= 18) &&
      !$"firstEvent".isNull && ($"firstEvent" === mid1 || $"firstEvent" <= left) &&
      !$"lastEvent".isNull && ($"lastEvent" > right || $"lastEvent" === mid2))
    ...
    val filtered_pre = exploded_pre.filter(
      $"time" =!= 0 && $"price" <= 18 &&
      ($"lastEvent" > right || $"lastEvent" === mid1) &&
      ($"firstEvent" === mid2 || $"firstEvent" <= left) &&
      $"price" > 5)
