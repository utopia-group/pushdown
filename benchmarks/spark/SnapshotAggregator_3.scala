...
object SnapshotAggregator_3 {
  def main(args: Array[String]): Unit = {
    val spark = SparkSession.builder
      .appName("SnapshotAggregator_3")
      .getOrCreate()
    ...
    // val diff = filtered.except(filtered_pre).union(filtered_pre.except(filtered))
    // if (diff.count() != 0) {
    //   throw new RuntimeException(
    //   s"Results differ: filtered and filtered_pre are not identical")
    // } else {
    //   // println(s"Results match: filtered and filtered_pre are identical")
    // }
