...
object MinPooling_4 {
  def main(args: Array[String]): Unit = {
    ...
    val filtered = out.filter(
      !$"row".getItem(0).isNull && ($"row".getItem(0) <= 29 || $"row".getItem(0) === 50) &&
      !$"row".getItem(1).isNull && ($"row".getItem(1) === 12 || $"row".getItem(1) < 10) &&
      !$"row".getItem(2).isNull && ($"row".getItem(2) === 15 || $"row".getItem(2) < 10))
