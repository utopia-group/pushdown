...
object MinPooling_7 {
  def main(args: Array[String]): Unit = {
    ...
    val filtered = out.filter(
      (!$"row".getItem(0).isNull && !($"row".getItem(0) > 0 && $"row".getItem(0) < 6) && $"row".getItem(0) <= 19) ||
      (!$"row".getItem(1).isNull && !($"row".getItem(1) === 12 || $"row".getItem(1) < 6) && $"row".getItem(1) < 20) ||
      (!$"row".getItem(2).isNull && !($"row".getItem(2) >= 0 && $"row".getItem(2) <= 6) && $"row".getItem(2) < 50))
