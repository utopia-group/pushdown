...
object KeepRowWithMaxQuality_8 {
  def main(args: Array[String]): Unit = {
    val spark = SparkSession.builder
      .appName("KeepRowWithMaxQuality_8")
      .getOrCreate()
    ...
    val filtered = exploded.filter(
      ((!col(udaf.KEY_SEQUENCE_IDENTIFIER).isNull && col(udaf.KEY_SEQUENCE_IDENTIFIER) >= 1) ||
       (!col(udaf.KEY_S_SEQUENCE_IDENTIFIER).isNull && col(udaf.KEY_S_SEQUENCE_IDENTIFIER) =!= 200)) &&
      !col(udaf.KEY_S_QUALITY_SCORE_IDENTIFIER).isNull && col(udaf.KEY_S_QUALITY_SCORE_IDENTIFIER) > 10 &&
      !col(udaf.KEY_S_QUALITY_SCORE).isNull && col(udaf.KEY_S_QUALITY_SCORE) > 1 &&
    ...
    val filtered_pre = exploded_pre.filter(
      (col(udaf.KEY_S_QUALITY_SCORE_IDENTIFIER).isNull || col(udaf.KEY_S_QUALITY_SCORE_IDENTIFIER) > 10) &&
      (col(udaf.KEY_S_QUALITY_SCORE).isNull || col(udaf.KEY_S_QUALITY_SCORE) > 1) &&
      (col(udaf.KEY_SEQUENCE_IDENTIFIER).isNotNull || col(udaf.KEY_S_SEQUENCE_IDENTIFIER).isNotNull) &&
      (col(udaf.KEY_SEQUENCE_IDENTIFIER).isNull || col(udaf.KEY_SEQUENCE_IDENTIFIER) >= 1 ||
       col(udaf.KEY_S_SEQUENCE_IDENTIFIER).isNull || col(udaf.KEY_S_SEQUENCE_IDENTIFIER) =!= 200))
