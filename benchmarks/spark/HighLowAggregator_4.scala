...
object HighLowAggregator_4 {
  def main(args: Array[String]): Unit = {
    val spark = SparkSession.builder
      .appName("HighLowAggregator_4")
      .getOrCreate()
    ...
    val ts2 = to_timestamp(lit("2020-07-30 08:00:00"), "yyyy-MM-dd HH:mm:ss")
    val filtered = exploded.filter(
			($"high_price".isNull || $"high_price" >= 10) &&
			($"high_price_ts".isNull || $"high_price_ts" <= ts2) &&
