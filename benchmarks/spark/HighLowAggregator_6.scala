...
object HighLowAggregator_6 {
  def main(args: Array[String]): Unit = {
    val spark = SparkSession.builder
      .appName("HighLowAggregator_6")
      .getOrCreate()
    ...
    val ts3 = to_timestamp(lit("2020-07-30 08:00:00"), "yyyy-MM-dd HH:mm:ss")
    val filtered = exploded.filter(
			($"high_price".isNull || $"high_price" <= 99) &&
			($"high_price_ts".isNull || ($"high_price_ts" === ts3 || $"high_price_ts" <= ts2)) &&
			($"low_price".isNull || $"low_price" > 10) &&
			($"low_price_ts".isNull || $"low_price_ts" > ts1))
