...
object EventAggregationUdf_8 {
  def main(args: Array[String]): Unit = {
    val spark = SparkSession.builder
      .appName("EventAggregationUdf_8")
      .getOrCreate()
    ...
    val mid = to_timestamp(lit("1990-07-30 08:00:00"), "yyyy-MM-dd HH:mm:ss")
    ...
    val filtered = exploded.filter(
      ($"time" > 0 || $"price" < 18) &&
      !$"firstEvent".isNull && ($"firstEvent" === mid || $"firstEvent" < left) &&
    ...
    val filtered_pre = exploded_pre.filter(
      $"lastEvent" > right &&
      ($"time" > 0 || $"price" < 18) &&
      ($"firstEvent" === mid || $"firstEvent" < left))
