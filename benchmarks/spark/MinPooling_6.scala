...
object MinPooling_6 {
  def main(args: Array[String]): Unit = {
    ...
    val filtered = out.filter(
      (!$"row".getItem(0).isNull && $"row".getItem(0) =!= 10 && $"row".getItem(0) <= 29) ||
      (!$"row".getItem(1).isNull && $"row".getItem(1) =!= 0 && $"row".getItem(1) < 10) ||
      (!$"row".getItem(2).isNull && $"row".getItem(2) =!= 15 && $"row".getItem(2) < 50))
