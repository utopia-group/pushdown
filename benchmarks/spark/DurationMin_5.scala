...
object DurationMin_5 {
  def main(args: Array[String]): Unit = {
    val spark = SparkSession.builder
      .appName("DurationMin_5")
      .getOrCreate()
    ...
    val predicate = daysUDF($"duration") <= 65
    ...
    val filtered = out.filter(
      !$"row".isNull &&
      monthsUDF($"row") >= 3 && monthsUDF($"row") <= 23 &&
      (daysUDF($"row") === 65 || daysUDF($"row") < 60) &&
      (microsUDF($"row") > 26 || microsUDF($"row") === 0))
    ...
    val filtered_pre = out_pre.filter(
      $"row".isNotNull &&
      (microsUDF($"row") > 26 || microsUDF($"row") === 0) &&
      (daysUDF($"row") === 65 || microsUDF($"row") < 60) &&
      monthsUDF($"row") >= 3 && monthsUDF($"row") <= 23)
    ...
    // val filteredRows = filtered.select("row").collect().map(_.get(0)).toSet
    // val filteredPreRows = filtered_pre.select("row").collect().map(_.get(0)).toSet

    // if (filteredRows == filteredPreRows) {
    //   // println("filtered and filtered_pre are equivalent.")
    // } else {
    //   // println(s"filtered and filtered_pre are NOT equivalent. Difference size: ${(filteredRows diff filteredPreRows).size}")
    // }
